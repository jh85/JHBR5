...
def process_chunk(args):
    """Process a range of PSV records from one file."""
    psv_path, start_offset, num_records, shard_id, output_dir, eval_coef = args

    records = np.fromfile(psv_path, dtype=cshogi.PackedSfenValue,
    ...
    for i in range(actual):
        try:
            ...
            # Decode position with cshogi
            board.set_psfen(np.array(r['sfen'], dtype=np.uint8))
            sfen = board.sfen()
            flip = sfen.split()[1] == 'w'

            # Encode planes (our Python encoder — proven correct)
            planes = sfen_to_planes(sfen)

            # Decode move with cshogi
            move = int(r['move'])
            if move == 0:
                errors += 1
                continue
            ...
            # WDL target (blend score + game result)
            score = int(r['score'])
...
def main():
    ...
    print(f"Found {len(psv_files)} PSV file(s)")
    ...
    for psv_path in psv_files:
        ...
        while offset < file_records:
            ...
            if chunk <= 0:
                break
            tasks.append((psv_path, offset * RECORD_SIZE, chunk, shard_id,
                          args.output_dir, args.eval_coef))
            offset += chunk
