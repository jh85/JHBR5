...
# ---------------------------------------------------------------------
# 100 representative positions from real games — verbatim from dlshogi's
# benchmark.py. Each entry is the move sequence from startpos as a
# space-separated USI move string.
# ---------------------------------------------------------------------
...
def main():
    ...
    def read_until(target_substr, timeout=120.0):
        deadline = time.time() + timeout
        last_info = None
        while True:
            if time.time() > deadline:
                raise TimeoutError(f"timeout waiting for '{target_substr}'")
            line = proc.stdout.readline()
            if not line:
                raise EOFError(f"engine exited before '{target_substr}'")
            line = line.rstrip('\n')
    ...
    print('Loading model (this can take a minute on first run for TRT)...',
          flush=True)
    t0 = time.time()
    send('isready')
    read_until('readyok', timeout=300)
    print(f'  isready in {time.time()-t0:.1f}s\n', flush=True)

    positions = POSITIONS[:args.limit] if args.limit else POSITIONS
    print(f'Running {len(positions)} positions...\n', flush=True)
    ...
    for i, moves in enumerate(positions):
        ...
        send(f'go byoyomi {args.byoyomi}')
        try:
            last_info = read_until('bestmove',
                                   timeout=args.byoyomi / 1000.0 * 5 + 10)
        except (TimeoutError, EOFError) as e:
            print(f'  [{i+1:3d}/{len(positions)}] FAILED: {e}', flush=True)
            failed += 1
            continue

        nps_m   = re.search(r'\bnps (\d+)',   last_info or '')
        nodes_m = re.search(r'\bnodes (\d+)', last_info or '')
        time_m  = re.search(r'\btime (\d+)',  last_info or '')

        nps   = int(nps_m.group(1))   if nps_m else None
        ...
        t_str  = f'{tm:>5}ms'   if tm is not None else '    ?ms'
        print(f'  [{i+1:3d}/{len(positions)}] '
              f'nps={nps_str}  nodes={n_str}  time={t_str}', flush=True)
    ...
    print('Summary')
    print('=' * 60)
    print(f'Positions completed : {len(nps_results)}/{len(positions)}'
          f' (failed: {failed})')
    print(f'Total bench time    : {bench_elapsed:.1f}s')
    if nps_results:
        nps_results.sort()
        print()
        print(f'NPS:')
        ...
        print(f'  max    : {max(nps_results):>12,}')
        # Percentiles for tail behavior
        n = len(nps_results)
        p10 = nps_results[int(n * 0.10)]
        p90 = nps_results[int(n * 0.90)]
        print(f'  p10    : {p10:>12,}')
        print(f'  p90    : {p90:>12,}')
    if nodes_results:
        print()
