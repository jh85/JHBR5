...
SearchWorker::NodeToProcess SearchWorker::PickNodeToExtend() {
  ...
  // Shared lock: multiple workers walk concurrently. Per-parent
  // synchronization on the child sibling-list is provided by
  // bucket-hashed spawn_lock inside Edge_Iterator::GetOrSpawnNode.
...
void SearchWorker::GatherMinibatchBulk() {
  PickNodesToExtend(config_.minibatch_size);
  ProcessPickedNodes();
}
