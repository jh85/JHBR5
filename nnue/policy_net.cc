...
PolicyScratch::PolicyScratch(const PolicyNet& net)
    : net_(&net), l1_(net.l1()), acc_(net.l1()), hl_(net.l1() / 2) {}
...
float PolicyScratch::Logit(const ShogiBoard& board, Move m) const {
  ...
  const int8_t* row = net_->out_w + static_cast<size_t>(bucket) * half;
  const int32_t dot = simd::DotI16I8(hl_.data(), row, half);
